...
- Reverted counting to single authoritative camera (COUNTING_CAMERA_ID = 0).
  Camera 1 still runs detection, tracking, and overlay drawing but does
  not increment counters or fire TTS. Removes BriefIDLock — single
  camera inherently prevents cross-camera double counts.
- Pipeline resolution fixed: 1024x576 (was 640x360 which caused
  not-negotiated GStreamer error on these cameras).
...
def get_global_track_id(camera_id, local_track_id, features=None, label=None):
    ...
    physical object seen by both cameras, which helps display continuity
    on camera 1 even though only camera 0 counts.
    """
...
def detection_callback(pad, info, callback_data):
    ...
    Only COUNTING_CAMERA_ID (camera 0) increments counters and fires TTS.
    Camera 1 still runs full detection, tracking, and overlay drawing.
    ...
    for detection in detections:
        ...
        # Only the authoritative camera (COUNTING_CAMERA_ID) increments
        # counters and fires TTS. Camera 1 still tracks and draws overlays.
...
def main():
    ...
    print(f"Counting camera:   {COUNTING_CAMERA_ID}  (camera 1: tracking + overlay only)")
