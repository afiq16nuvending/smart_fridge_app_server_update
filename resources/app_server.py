...
# Minimum detection confidence for a detection to FEED the counter. Only the
# noisiest flickering boxes (glare, frame edges) are dropped. Keep this LOW -
# the foreshortened left side produces legitimately low-confidence boxes that
# we still want to count, so do not raise this much above 0.3 without
# checking the [LOWCONF] log first.
COUNT_CONF_MIN = 0.30

# Anchor re-stitch across tracker ID churn. When a BRAND-NEW global_id
# appears very close to a different track that was active a few frames ago
# with the SAME label, we treat it as the same physical item whose ID the
# tracker churned, and INHERIT its anchor + last direction instead of
# resetting. This keeps displacement accumulating across churn (the L:40->
# 41->42 flicker seen in testing) so a grab is not split into sub-threshold
# fragments. Guarded so it cannot merge two items that are both active.
STITCH_DIST_PX        = 70   # max |dx|+|dy| (px) to treat as the same item
...
class HailoDetectionCallback(app_callback_class):
    ...
    def get_fallback_pipeline_string(self):
        ...
        return (
            ...
            "hailotracker name=hailo_tracker class-id=-1 kalman-dist-thr=0.8 iou-thr=0.9 init-iou-thr=0.7 keep-new-frames=1 keep-tracked-frames=1 keep-lost-frames=15 keep-past-metadata=true ! "
...
class NetDisplacementCounter:
    ...
    def _find_stitch(self, global_id, x, y, label):
        ...
        Returns its global_id or None. Guards: not the same id, same label,
        NOT active this frame (so two coexisting items can't merge), within
        the gap window, and nearest within stitch_dist."""
...
def commit_net_count(user_data, global_id, label, direction,
    ...
    td = user_data.tracking_data
    td.class_counters[direction][label] += 1
    ...
    td.counted_tracks.setdefault(direction, set()).add(global_id)

    validation_result = user_data.validate_detected_product(label)
    if validation_result['valid']:
        slot = td.validated_products[direction].setdefault(
            label, {"count": 0, "product_details": validation_result['product_details']}
        )
        slot["count"] += 1
    else:
        slot = td.invalidated_products[direction].setdefault(
            label, {"count": 0, "raw_detection": {
                "name": label, "confidence": 0.0,
                "tracking_id": global_id, "bounding_box": {}
            }}
        )
        slot["count"] += 1
...
def main():
    ...
    print(f"Conf floor:        {COUNT_CONF_MIN:.2f}  (below = not counted)")
    print(f"ID re-stitch:      <= {STITCH_DIST_PX}px, <= {STITCH_MAX_GAP_FRAMES} "
          f"frame gap (anti-churn)")
